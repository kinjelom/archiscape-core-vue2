<template>
  <div>
    <h2 id="page-heading" data-cy="TeamHeading">
      <span v-text="$t('archiscapeCoreApp.team.home.title')" id="team-heading">Teams</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('archiscapeCoreApp.team.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'TeamCreate' }" custom v-slot="{ navigate }">
          <button @click="navigate" id="jh-create-entity" data-cy="entityCreateButton" class="btn btn-primary jh-create-entity create-team">
            ...
            <span v-text="$t('archiscapeCoreApp.team.home.createLabel')"> Create a new Team </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && teams && teams.length === 0">
      <span v-text="$t('archiscapeCoreApp.team.home.notFound')">No teams found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('archiscapeCoreApp.team.description')">Description</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="archiscapeCoreApp.team.delete.question" data-cy="teamDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-team-heading" v-text="$t('archiscapeCoreApp.team.delete.question', { id: removeId })">
